...
define('LIQUIDWEB_HOOKS_CLONE_FROME_SERVER_FIELD_NAME', 'Clone From Server');
define('LIQUIDWEB_HOOKS_SERVER_TYPE', 'LiquidWeb');

function liquidweb_hook_cart($cart){

   //LIQUIDWEB_HOOKS_SERVER_TYPE
   //if(!isset($_SESSION['cartdomain']) || empty($_SESSION['cartdomain']) || !isset($_SESSION['uid']) || !$_SESSION['uid']){
   ...
   $rq = mysql_query('SELECT * FROM `tblproducts` WHERE servertype = "'.LIQUIDWEB_HOOKS_SERVER_TYPE.'"');
   ...
   foreach($cart['products'] as $product){
      ...
      //if(!isset($product['domain']) || !$product['domain'] || strcmp($product['domain'], $domainName) !== 0 || empty($product['customfields']) || in_array((int)$product['pid'],$SystemProductsIds) === false){
   ...
   while(($res = mysql_fetch_assoc($q))){
   	 if(strcmp($res['fieldname'], LIQUIDWEB_HOOKS_CLONE_FROME_SERVER_FIELD_NAME) === 0){
   	 	$customField = $res;
		break;
   	 }
   }
   ...
   $allHosting = StormOnDemand_Helper::getAllVpsUserUniqIds($_SESSION['uid'],StormOnDemand_Helper::LiquidWebLiquidWebServerType,false);
...
function liquidweb_hook_admin_user_order($conf){
	...
	if(!isset($_REQUEST['userid'])  || !StormOnDemand_Helper::isLiquidWebVpsCloneServerField($conf['fieldid'],$idHosting)){
		return;
	}

    $allHosting = StormOnDemand_Helper::getAllVpsUserUniqIds($idUser,StormOnDemand_Helper::LiquidWebLiquidWebServerType,false);
    if(!empty($allHosting)){
   		foreach($allHosting as $hosting){
   			$hostingList []= array(
				'id'   => $hosting['uniq_id'],
...
//Fix for double clone serwers
function liquidweb_ClientAreaHeadOutput(){
...
add_hook("PreCalculateCartTotals",1,"liquidweb_hook_cart");
add_hook("CustomFieldLoad",1,"liquidweb_hook_admin_user_order");
add_hook("ClientAreaHeadOutput",1,"liquidweb_ClientAreaHeadOutput");
?>