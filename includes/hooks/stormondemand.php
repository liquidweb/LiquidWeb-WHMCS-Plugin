...
define('STOD_HOOKS_CLONE_FROME_SERVER_FIELD_NAME', 'Clone From Server');
define('STOD_HOOKS_SERVER_TYPE', 'StormOnDemand');

function stormondemand_hook_cart($cart){
   ...
   $rq = mysql_query('SELECT * FROM `tblproducts` WHERE servertype = "'.STOD_HOOKS_SERVER_TYPE.'"');
   ...
   while(($res = mysql_fetch_assoc($q))){
   	 if(strcmp($res['fieldname'], STOD_HOOKS_CLONE_FROME_SERVER_FIELD_NAME) === 0){
   	 	$customField = $res;
		break;
   	 }
   }
   ...
   /* GETTING ALL, ACTIVE WITH uniq_id USER STORMONDEMAND SERVERS */
   $hostingList = array();
   ...
   $allHosting = StormOnDemand_Helper::getAllVpsUserUniqIds($_SESSION['uid'],StormOnDemand_Helper::StormOnDemandStormOnDemandServerType,false);
   if(!empty($allHosting)){
...
function stormondemand_hook_admin_user_order($conf){
	...
	if(!isset($_REQUEST['userid']) || !StormOnDemand_Helper::isStormOnDemandVpsCloneServerField($conf['fieldid'],$idHosting)){
		return;
	}			
	
    $allHosting = StormOnDemand_Helper::getAllVpsUserUniqIds($idUser,StormOnDemand_Helper::StormOnDemandStormOnDemandServerType,false);
    if(!empty($allHosting)){
   		foreach($allHosting as $hosting){
   			$hostingList []= array(
				'id'   => $hosting['uniq_id'],
...
//Fix for double clone serwers
function stormondemand_ClientAreaHeadOutput(){
...
add_hook("PreCalculateCartTotals",1,"stormondemand_hook_cart");
add_hook("CustomFieldLoad",1,"stormondemand_hook_admin_user_order");
add_hook("ClientAreaHeadOutput",1,"stormondemand_ClientAreaHeadOutput");
?>