...
if(file_exists(dirname(__FILE__).DS.'moduleVersion.php')){
    require_once dirname(__FILE__).DS.'moduleVersion.php';
     define('STORM_ON_DEMAND_LOAD_BALANCER_VERSION', $moduleVersion);
}else{
...
function StormOnDemandLoadBalancer_ConfigOptions()
{
    ...
    elseif($_REQUEST['stormajax'] == 'generate-confoption')
    {
        ...
        $groups[] = array
        (
            'Name'          =>  'Configurable Options For Storm On Demand Load Balancer',
    ...
    mysql_query("CREATE TABLE IF NOT EXISTS `mg_storm_on_demand_load_balancer`
        (
    ...
    $newVersion = StormOnDemandLoadBalancer_getLatestVersion();
    ...
    if($newVersion && $script == 'configproducts.php' && $_GET['action'] != 'save')
    {
        echo '<p style="text-align: center;" class="infobox op_version">
            <span style="font-weight: bold">New version of Storm On Demand Load Balancer module is available!</span>
    ...
    if($script == 'configproducts.php'){
       $testConnection = StormOnDemandLoadBalancer_checkConnection();
    }else{
       $testConnection = true;
    }
...
function StormOnDemandLoadBalancer_CreateAccount($params)
{
    //get configuration
    $username   =   StormOnDemandLoadBalancer_getOption('Username', $params);
    $password   =   StormOnDemandLoadBalancer_getOption('Password', $params);
    $strategy   =   StormOnDemandLoadBalancer_getOption('Strategy', $params);
    $sess_per   =   StormOnDemandLoadBalancer_getOption('Session Persistence', $params) ? 1 : 0;
    $hostname   =   $params['customfields']['hostname'] ? $params['customfields']['hostname'] : $params['domain'];
    $region     =   StormOnDemandLoadBalancer_getOption('Region', $params);
    ...
    $ret = $balancer->create($hostname, array(array('src_port' =>  StormOnDemandLoadBalancer_getOption('Default Source Port', $params), 'dest_port' => StormOnDemandLoadBalancer_getOption('Default Destination Port', $params))), $strategy, $sess_per, $region);
    ...
    mysql_query("REPLACE INTO mg_storm_on_demand_load_balancer (`hosting_id`, `uniq_id`) VALUES ('".$params['serviceid']."', '".$ret['uniq_id']."')") or die(mysql_error());
    //return successful message
    return "success";
}

function StormOnDemandLoadBalancer_TerminateAccount($params)
{
    //get configuration
    $username   =   StormOnDemandLoadBalancer_getOption('Username', $params);
    $password   =   StormOnDemandLoadBalancer_getOption('Password', $params);
    ...
    $q = mysql_query("SELECT * FROM mg_storm_on_demand_load_balancer WHERE hosting_id = ".(int)$params['serviceid']);
    ...
    mysql_query("DELETE FROM mg_storm_on_demand_load_balancer WHERE hosting_id = ".(int)$params['serviceid']);
    return "success";
}

function StormOnDemandLoadBalancer_ClientAreaCustomButtonArray()
{
    $return = array
    (
        'Settings'      =>  'settings',
...
function StormOnDemandLoadBalancer_Settings($params)
{
    //get configuration
    $username       =   StormOnDemandLoadBalancer_getOption('Username', $params);
    $password       =   StormOnDemandLoadBalancer_getOption('Password', $params);
    $max_nodes      =   StormOnDemandLoadBalancer_getOption('Number Of Nodes', $params) ? StormOnDemandLoadBalancer_getOption('Number Of Nodes', $params) : 0;
    $max_services   =   StormOnDemandLoadBalancer_getOption('Number Of Services', $params) ? StormOnDemandLoadBalancer_getOption('Number Of Services', $params) : 0;
    $ssl            =   StormOnDemandLoadBalancer_getOption('SSL', $params);
    $hostname       =   $params['customfields']['hostname'] ? $params['customfields']['hostname'] : $params['domain'];
                ...
                if(count($_REQUEST['nodes']))
                {
                    ...
                    if($nodes['items'])
                    {
                        ...
                        $private_servers = mysql_get_array("SELECT h.id
                            ...
                            WHERE h.userid = ? AND p.servertype = ?", array($_SESSION['uid'], 'StormOnDemandPrivateParent'));
...
function StormOnDemandLoadBalancer_ChangePackage($params)
{
    //get configuration
    $username       =   StormOnDemandLoadBalancer_getOption('Username', $params);
    $password       =   StormOnDemandLoadBalancer_getOption('Password', $params);
...
function StormOnDemandLoadBalancer_ClientArea($params)
{
    //get configuration
    $username   =   StormOnDemandLoadBalancer_getOption('Username', $params);
    $password   =   StormOnDemandLoadBalancer_getOption('Password', $params);
    ...
    $smarty->assign('buttons', StormOnDemandLoadBalancer_ClientAreaCustomButtonArray());
...
function StormOnDemandLoadBalancer_AdminServicesTabFields($params)
{
    //get configuration
    $username   =   StormOnDemandLoadBalancer_getOption('Username', $params);
    $password   =   StormOnDemandLoadBalancer_getOption('Password', $params);
...
function StormOnDemandLoadBalancer_getOption($option, $params)
{
    $config = StormOnDemandLoadBalancer_ConfigOptions();
...
StormOnDemandLoadBalancer_registerInstance();
function StormOnDemandLoadBalancer_registerInstance()
{
    /****************************************************
     *              EDIT ME
     ***************************************************/
    //Set up name for your module.
    $moduleName         =   'Storm On Demand Load Balancer For WHMCS';
    //Set up module version. You should change module version every time after updating source code.
    $moduleVersion      =   STORM_ON_DEMAND_LOAD_BALANCER_VERSION;
    //Encryption key
    $moduleKey          =   'WuIokP30ovuS4sNaHEZFznPcoswJoLz7cmv7cRIOqX98lmUZ1aybAEvmq7GtXBLD';
    /***************************************************
     *                      DO NOT TOUCH!
     ***************************************************/
...
function StormOnDemandLoadBalancer_getLatestVersion()
{
    /****************************************************
     *              EDIT ME
     ***************************************************/
