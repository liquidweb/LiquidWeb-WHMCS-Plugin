jQuery(function(){
	...
	function checkQuota(zoneValue){
		if(!zoneValue){
			...
			jQuery('select[name="{$inpt_bandwidth_quota_name}"]').val('')
															.html('<option value="-1">Please, select Zone</option>')
															.attr('disabled','disabled');
			
			
			//change Backup Quota
			jQuery('select[name="{$inpt_backup_quota_name}"]').val('')
															.html('<option value="-1">Please, select Zone</option>')
															.attr('disabled','disabled');												
		}else{
			//change Bandwidth Quota
			jQuery('select[name="{$inpt_bandwidth_quota_name}"]').html('').removeAttr('disabled');
			jQuery('select[name="{$inpt_bandwidth_quota_name}"]').parent().append('<img src="../modules/servers/StormOnDemand/assets/images/admin/loading.gif" class="preloader" alt="loading..."/>');
			
			
			//change Backup Quota
			jQuery('select[name="{$inpt_backup_quota_name}"]').html('').removeAttr('disabled');
			jQuery('select[name="{$inpt_backup_quota_name}"]').parent().append('<img src="../modules/servers/StormOnDemand/assets/images/admin/loading.gif" class="preloader" alt="loading..."/>');
				...
				if(data && data !== ''){
				    try{
						...
						jQuery('select[name="{$inpt_bandwidth_quota_name}"]').val(bandwidthQuota).on('change', function(){
							bandwidthQuota = parseInt(jQuery(this).val());
						});
      				...
      				switch(productTemplates['templates_info'][iOption]['type']){
      					...
      					case 'select':
      						...
      						if(iOption === 'backup_quota'){
      							...
      							bandwidthQuota = parseInt(valToSet);
      						}else{
    ...
    if (jQuery('input[name="{$inpt_zone_name}"]').val() == ''){
        checkQuota({$input_zone_default_id} , true);
    }else{
        checkQuota(jQuery('input[name="{$inpt_zone_name}"]').val(), true);
    }
});
