...
//define('StormOnDemandCacheLive',21600); //6h
define('StormOnDemandCacheLive', 1); //6h
define('StormOnDemandCacheName', 'stormondemandforwhmcs_quota_cache');
define('StormOnDemandConfigName', 'stormondemandforwhmcs_config_cache');
define('StormOnDemandDefaultZone', 27);
define('StormOnDemandFirewallSlashEscapeChar', '_');

if(file_exists(dirname(__FILE__).DS.'moduleVersion.php')){
    require_once dirname(__FILE__).DS.'moduleVersion.php';
     define('STORM_ON_DEMAND_VERSION', $moduleVersion);
}else{
    ...
    if(strpos($_SERVER['SCRIPT_FILENAME'], 'configproducts.php') !== false)
    {
        ...
        if(! empty($username) && ! empty($password))
        {
            ...
            if ($error = $config->getError()) {
                logModuleCall('StormOnDemand','checkConnection',$config->getLastRequest(),'',$config->getLastResponse(),array());
            }
...
function StormOnDemand_ConfigOptions()
{
    ...
    if ($_REQUEST['stormajax'] == 'load-config')
    {
        ...
        if (!$zoneSelected) {
            $zoneSelected = (int) StormOnDemandDefaultZone;
        }
        ...
        $confs   = array();
        $baCache = ModuleInformationClient::getWHMCSconfig(StormOnDemandConfigName);
        if ($baCache) {
            $baCache = json_decode($baCache, true);
        }

        //if there is no data in cache:
        if (!$baCache || !isset($baCache['cache_time']) || (((int)time() - (int)$baCache['cache_time']) > StormOnDemandCacheLive)) {
            ...
            //has error?
            if ($error = $config->getError()) {
                logModuleCall('StormOnDemand','ConfigOptions',$config->getLastRequest(),'',$config->getLastResponse(),array());
            }
            ...
            if (isset($count['item_total']) && $count['item_total'] > 0)
            {
                ...
                $baCache['data'] = $confs;
                //ModuleInformationClient::saveWHMCSconfig(StormOnDemandConfigName,json_encode($baCache));//NTODO: odkomentowa�?
            } else {
                $confs = $baCache['data'];
            }
    ...
    elseif ($_REQUEST['stormajax'] == 'load-image')
    {
        ...
        {//NTODO - dalej powstawiać logowanie errorów
            logModuleCall('StormOnDemand','ConfigOptions',$image->getLastRequest(),'',$image->getLastResponse(),array());
    ...
    elseif ($_REQUEST['stormajax'] == 'generate-confoption')
    {
        ...
        //Server//NTODO: to jest tu chyba zbędne bo nie mamy tu unique_id bo żadnego serwera nie zakładamy przecież, ani nie musimy mieć założonego
        ...
        //NTODO: uzupełnić tutaj o if (error)
        logModuleCall('StormOnDemand','ConfigOptions',$template->getLastRequest(),'',$template->getLastResponse(),array());
        ...
        //Images
        $configurable_options[2] = array//NTODO: nie ma rezultatu - sprawdzić czy jest ok i nie ma na serwerze, czy trzeba coś ulepszyć/zmienić
        (
        ...
        //NTODO:   zones: /items/region/name
        //NTODO:   products: /options/key|values|description|options
        //NTODO:   products/lists:  /items/code musi się zgadzać

        //NTODO

        //NTODO: Updated by Piotr kozdęba <piotr.ko@modulesgarden.com>
        //Backup Quota
       ...
       // die(print_r($product_ret,true));
        ...
        //NTODO - koniec poprzedniego NTODO

      //  die(print_r($server->details(),true));return;
...
/*
        ...
        (
            ...
            'Values'  => array
            (
                '100' => '100GB',//NTODO: by�o 10/20/30 - moze to lepiej zdziera� z serwera?
                '200' => '200GB',
                '500' => '500GB'
            )
        );*/
//NTODO: odkomentowałem, było://nie ma Daily backup quota - tak ma by� (poni�erj kopia z liquid weba)?
      //Daily Backup Quota
                        ...
                        if (strcmp($vOpt['key'], 'Bandwidth') === 0) {
                            ...
                            if (!empty($vOpt['values'])) {
                                foreach ($vOpt['values'] as $kVal => $vVal) {
                                    ...
                                    $configurable_options[10]['Values'][$limit] = $limit.' GB';
                                }
                            }
        ...
        (
            ...
            'Type'      => 'select',
            'Values'    => array
            (//NTODO: w liquid web to jest inaczej - tak ma by�?
                'Pay as You Go' => 'Pay as You Go',
                '250'   => '250',
                '500'   => '500',
                '1000'  => '1000',
        ...
        */
        ...
        $groups[] = array
        (
            'Name'          => 'Configurable Options For Storm On Demand Servers',
    ...
    } elseif ($_REQUEST['stormajax'] == 'load-quota')
    {
        ...
        if (empty($_REQUEST) || !isset($_REQUEST['id']) || !preg_match('/^[0-9]{1,}$/D', $_REQUEST['id'])) {
            ...
            $baCache = ModuleInformationClient::getWHMCSconfig(StormOnDemandCacheName);//NTODO: odkomentowałem

            if ($baCache) {//NTODO: odkomentowalem
                 $baCache = json_decode($baCache, true);
            }
            ...
            if (!$baCache || !isset($baCache['cache_time']) || (((int)time() - (int)$baCache['cache_time']) > StormOnDemandCacheLive)) {
                ...
                $baCache['data']['bandwidth_quota'][0] = array(
...
/*NTODO: liquid web ma to odkomentowane:*///ModuleInformationClient::saveWHMCSconfig(StormOnDemandCacheName, json_encode($baCache));
            }
    ...
    mysql_query("CREATE TABLE IF NOT EXISTS `mg_storm_on_demand`
        (
    ...
    (
        ...
        (
            'Type'          => 'dropdown',//NTODO: tu s� r�ne rzeczy - inne ni� w configurable options czy gdzies tam wyzej - posprawdzac to dobrze
       //NTODO: zakomentowałem     'Options'       => 'Pay as You Go,250,500,1000,2000,4000',//NTODO: wyrzucić?
        ),
        'Monitoring'        => array
        (
            'Type'          => 'yesno',
    ...
    $jsTplParams  = array('id' => $productId,
        'backup_quota'    => 0,
        'bandwidth_quota' => 0,
       /*NTODO: tego nie ma w liquid web*/ 'template'        => 0,
        'zone_name'       => 'Zone C (US Central)',
        'templates'       => array(),
      /*NTODO: tego nie ma w liquid web*/  'inpt_template_name' => 'packageconfigoption[' . (array_search('Template', $configFormKeys) + 1) . ']',
        ...
        'input_zone_default_id'       => StormOnDemandDefaultZone,
    );
    ...
    $templates = StormOnDemand_loadTemplates();
    ...
    $xmlTemplate = array(
        ...
        'template'       => array(
            'type'       => 'select',//NTODO: tu jest text w liquid web
    ...
    $config['Backup Quota']['Description'] = "<script type='text/javascript'>".StormOnDemand_loadAsset('js/ProductConfigure.tpl.js', $jsTplParams)."</script>";
    ...
    if (basename($_SERVER["SCRIPT_NAME"]) == 'configproducts.php')
    {
        ...
        $config['Backup Quota']['Description'] .= "<script type='text/javascript'>".StormOnDemand_loadAsset('js/LoadConfiguration.tpl.js', $lcConfig).'</script><div id="conf-dialog" style="display:none;" title=""></div>';
    }


    $newVersion = StormOnDemand_getLatestVersion();
    ...
    if(basename($_SERVER["SCRIPT_NAME"]) == 'configproducts.php'){
        $testConnection = StormOnDemand_checkConnection();
    }else{
        $testConnection = true;
    }
...
function StormOnDemand_CreateAccount($params)
{

    //get configuration
    $username        = StormOnDemand_getOption('Username', $params);
    $password        = StormOnDemand_getOption('Password', $params);
    $config          = StormOnDemand_getOption('Config', $params);
    $template        = StormOnDemand_getOption('Template', $params);
    ...
    if (isset($params['customfields']['Clone From Server']) && $params['customfields']['Clone From Server']) {
        ...
        if ($error = $server->getError()) {
            logModuleCall('StormOnDemand','CreateAccount',$server->getLastRequest(),'',$server->getLastResponse(),array());

            return $error;
        }

        //save uniq_id to database. We need it!
        mysql_query("REPLACE INTO mg_storm_on_demand (`hosting_id`, `uniq_id`) VALUES ('".$params['serviceid']."', '".$ret['uniq_id']."')");
    } else {
        //check bandwidth quota
        $configuration = StormOnDemand_ConfigOptions();
        ...
        if (StormOnDemand_getOption('Backup Enabled', $params) != null)
        {
            $configuration['backup_enabled'] = 1;
            /*NTODO:było: StormOnDemand_getOption('Backup Quota', $params);*/
            $configuration['backup_quota']   = StormOnDemand_getOption('Backup Plan', $params) == 'quota' ? StormOnDemand_getOption('Backup Quota', $params) : StormOnDemand_getOption('Daily Backup Quota', $params);
            $configuration['backup_plan']    = StormOnDemand_getOption('Backup Plan', $params);
        }else{
        ...
        //has error?
        if ($error = $server->getError())
        {//NTODO: dodać logModuleCall gdzie potrzeba
            logModuleCall('StormOnDemand','CreateAccount',$server->getLastRequest(),'',$server->getLastResponse(),array());

            return $error;
        }
        //save uniq_id to database. We need it!
        mysql_query("REPLACE INTO mg_storm_on_demand (`hosting_id`, `uniq_id`) VALUES ('".$params['serviceid']."', '".$ret['uniq_id']."')") or die(mysql_error());
...
function StormOnDemand_TerminateAccount($params)
{
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
    $password = StormOnDemand_getOption('Password', $params);
    ...
    mysql_query("DELETE FROM mg_storm_on_demand WHERE hosting_id = ".(int)$params['serviceid']);
    return "success";
}
 ...
 * @return type
 */
function StormOnDemand_AdminCustomButtonArray()
{
    return array(
        'Reboot'    => 'reboot',
        'Shutdown'  => 'shutdown',
...
function StormOnDemand_ClientAreaCustomButtonArray()
{
    $id = (int)$_REQUEST['id'];
    ...
    {
        //NTODO: updated by Piotr Kozdeba <piotr.ko@modulesgarden.com>
        $optionname = $row['optionname'];
        if(strpos($row['optionname'], '|'))
        {
            $exp = explode('|', $row['optionname'],2);
            $optionname = $exp[0];
        }
   ...
   $op_q = mysql_query("SELECT tblproducts.* FROM tblhosting LEFT JOIN tblproducts ON tblhosting.packageid = tblproducts.id WHERE tblhosting.id = " . (int)$id . " LIMIT 1");
    ...
    if(StormOnDemand_getOption("IPs Management", $params))
    {
      $return['IP Management'] = 'ipmanagement';
    }elseif($op_row['configoption16'] == 'on' && StormOnDemand_getOption("IPs Management", $params) == null){
      $return['IP Management'] = 'ipmanagement';
    }


    if(StormOnDemand_getOption("Firewall", $params))
    {
        $return['Firewall'] = 'firewall';
    }elseif($op_row['configoption15'] == 'on' && StormOnDemand_getOption("Firewall", $params) == null){
      $return['Firewall'] = 'firewall';
    }

    if(StormOnDemand_getOption("Backup Enabled", $params))
    {
        $return['Backups'] = 'backups';
    }elseif($op_row['configoption8'] == 'on' && StormOnDemand_getOption("Backup Enabled", $params) == null){
      $return['Backups'] = 'backups';
    }
...
function StormOnDemand_Reboot($params)
{
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
    $password = StormOnDemand_getOption('Password', $params);
...
function StormOnDemand_clientReboot($params)
{
    $vars = array();
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
...
function StormOnDemand_Shutdown($params)
{
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
    $password = StormOnDemand_getOption('Password', $params);
...
function StormOnDemand_ClientShutdown($params)
{
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
    $password = StormOnDemand_getOption('Password', $params);
    ...
    $links = StormOnDemand_ClientAreaCustomButtonArray();
    //$this->assign('storm_links', $links);
...
function StormOnDemand_Start($params)
{
    $vars = array();
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
...
function StormOnDemand_ClientStart($params)
{
    $vars = array();
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
...
function StormOnDemand_ChangePackage($params)
{
    //get configuration
    $username           = StormOnDemand_getOption('Username', $params);
    $password           = StormOnDemand_getOption('Password', $params);
    ...
    $configuration = array
    (
        'bandwidth_quota'   =>  ($bandwidth_quota == 'Pay as You Go') ? 0 : $bandwidth_quota,//NTODO : na liquid weba jest samo $bandwidth_quota
        'domain'            =>  $hostname
    );
    ...
    {
        $configuration['backup_enabled']    = 1;
        $configuration['backup_quota']      =/*NTODO: było: StormOnDemand_getOption('Backup Quota', $params); */StormOnDemand_getOption('Backup Plan', $params) == 'quota' ? StormOnDemand_getOption('Backup Quota', $params) : StormOnDemand_getOption('Daily Backup Quota', $params);
        $configuration['backup_plan']       = StormOnDemand_getOption('Backup Plan', $params);
    }

    $Firewall = StormOnDemand_getOption("Firewall", $params);
    ...
    $server->resize($uniq_id, StormOnDemand_getOption('Config', $params), 0);
    if ($error = $server->getError())
    {
        return $error;
    }
...
function StormOnDemand_ClientArea($params)
{
    //get configuration
    $username   = StormOnDemand_getOption('Username', $params);
    $password   = StormOnDemand_getOption('Password', $params);
    $monitoring = StormOnDemand_getOption('Monitoring', $params);
    ...
    $smarty->assign('buttons', StormOnDemand_ClientAreaCustomButtonArray());
...
function StormOnDemand_AdminServicesTabFields($params)
{
    //NTODO: poni�sze 2 linir tylko tutaj:
    $qProduct = mysql_query("SELECT * FROM tblproducts WHERE id = " . (int)$params['pid'] . " LIMIT 1");
    $rowProduct = mysql_fetch_assoc($qProduct);
            ...
            echo '<script type="text/javascript">
                    $(function(){
                        $(".history-change-page").click(function(event){
                            ...
                            $("#storm-history").html("<p style=\"text-align:center\"><img src=\"../modules/servers/StormOnDemand/assets/images/admin/loading.gif\" alt=\"loading...\"/></p>");
            ...
            echo '<script type="text/javascript">
                    $(function(){
                        $(".image-change-page").click(function(event){
                            ...
                            $("#storm-images").html("<p style=\"text-align:center\"><img src=\"../modules/servers/StormOnDemand/assets/images/admin/loading.gif\" alt=\"loading...\"/></p>");
                            $.post(document.location.toString()+"&"+$(this).attr("href"), function(data){
            ...
            echo '<script type="text/javascript">
                    $(function(){
                        $(".template-change-page").click(function(event){
                            ...
                            $("#storm-templates").html("<p style=\"text-align:center\"><img src=\"../modules/servers/StormOnDemand/assets/images/admin/loading.gif\" alt=\"loading...\"/></p>.");
                            $.post(document.location.toString()+"&"+$(this).attr("href"), function(data){
...
function StormOnDemand_AdminServicesTabFieldsSave($params)
{
    mysql_query("REPLACE INTO mg_storm_on_demand SET uniq_id = '".$_REQUEST['uniq_id']."', hosting_id=".(int)$params['serviceid']);
}

//NTODO: poni�sze zakomentowane tylko tutaj by�o
//function StormOnDemand_loadTemplateOptions()
//{
//    $conf_id = $_REQUEST['conf_id'];
//
//    $q = mysql_query("SELECT * FROM tblproducts WHERE id = " . $_REQUEST['id'] . " LIMIT 1");
//    $row = mysql_fetch_assoc($q);
//
//    $username = $row['configoption1'];
//    $password = $row['configoption2'];
...
//FIREWALL
function StormOnDemand_Firewall($params)
{
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
    $password = StormOnDemand_getOption('Password', $params);
        ...
        switch ($_POST['type'])
        {
            ...
            case 'basic':
                ...
                foreach ($options as &$value) {
                    $value = str_replace(StormOnDemandFirewallSlashEscapeChar, '/', $value);
                }
    ...
    if (!empty($vars['options'])) {
        foreach ($vars['options'] as $k => $v) {
            $vars['options'][$k] = str_replace('/', StormOnDemandFirewallSlashEscapeChar, $v);
        }
    }
    ...
    if ($vars['type'] == 'basic')
    {
        ...
        {
            $vars['rules'][str_replace('/', StormOnDemandFirewallSlashEscapeChar, $rule['label'])] = 1;
        }
    }
...
//IP Management
function StormOnDemand_IPManagement($params)
{
    //get configuration
    $username   =   StormOnDemand_getOption('Username', $params);
    $password   =   StormOnDemand_getOption('Password', $params);
...
//Backups
function StormOnDemand_Backups($params)
{
    //get configuration
    $username = StormOnDemand_getOption('Username', $params);
    $password = StormOnDemand_getOption('Password', $params);
...
function StormOnDemand_Restore($params)
{
    //get configuration
    $username   =   StormOnDemand_getOption('Username', $params);
    $password   =   StormOnDemand_getOption('Password', $params);
    ...
    require_once ROOTDIR . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'StormOnDemand' . DIRECTORY_SEPARATOR . 'modulesgarden' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'class.StormOnDemand_Helper.php';
    $idUniqs = StormOnDemand_Helper::getAllVpsUserUniqIds($params['clientsdetails']['userid'], StormOnDemand_Helper::StormOnDemandStormOnDemandServerType);
    ...
    $vars['servers'] = StormOnDemand_Helper::getAllVpsUserUniqIds($params['clientsdetails']['userid'], StormOnDemand_Helper::StormOnDemandStormOnDemandServerType, false);
            ...
            case 'restore':
                switch ($_REQUEST['type'])
                {
                    ...
                    case 'server':
                        ...
                        require_once ROOTDIR . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'StormOnDemand' . DIRECTORY_SEPARATOR . 'modulesgarden' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR . 'class.StormOnDemand_Helper.php';
                        $rAllUniqsId = StormOnDemand_Helper::getAllVpsUserUniqIds($params['clientsdetails']['userid'], StormOnDemand_Helper::StormOnDemandStormOnDemandServerType/*NTODO: na liquid web by�o tak samo jak tutaj - to chyba b��d by�*/);
                        ...
                        if ($uniq_id && $error = $apiServer->getError()) {
                            ...
                            if ($error = $apiServer->getError()) {
                                ...
                                ModuleInformationClient::mysql_safequery("REPLACE INTO mg_storm_on_demand (`hosting_id`, `uniq_id`) VALUES (?,?)", array(
                                    $params['serviceid'],
                                    $result['uniq_id'],
                                ));
...
function StormOnDemand_History($params)
{
    //get configuration
    $username   =   StormOnDemand_getOption('Username', $params);
    $password   =   StormOnDemand_getOption('Password', $params);
...
function StormOnDemand_BlockStorage($params) {
    ...
    if (StormOnDemand_validateArray($_GET, array(
                array(
                    'field' => 'ajaxaction',
                    'preg'  => '/^[a-z]{1,10}$/D',
                ),
        ...
        $soadUserProductQuery = ModuleInformationClient::mysql_safequery('SELECT hosting.id AS hosting_id,hosting.orderid as hosting_order_id, hosting.domain as hosting_domain, sod.uniq_id
																	   	  FROM tblhosting AS hosting
																	   	  JOIN mg_storm_on_demand as sod ON hosting.id = sod.hosting_id
    ...
    $hostingUniqKeyQuery   = ModuleInformationClient::mysql_safequery('SELECT * FROM mg_storm_on_demand WHERE hosting_id = ? LIMIT 1', array($hosting_id));
...
function StormOnDemand_getOption($option, $params)
{
    $config = StormOnDemand_ConfigOptions();
...
StormOnDemand_registerInstance();

function StormOnDemand_registerInstance()
{
    /****************************************************
     *              EDIT ME
     ***************************************************/
    //Set up name for your module.
    $moduleName         =   'Storm On Demand For WHMCS';
    //Set up module version. You should change module version every time after updating source code.
    $moduleVersion      =   STORM_ON_DEMAND_VERSION;
    //Encryption key
    $moduleKey          =   'mGKUvqMqtKiTFkTPDkPLwOIrl1fcUk6qQcwl2KXk7GR338kvU7dTefMfbyEhhEii';
...
function StormOnDemand_getLatestVersion()
{
    /****************************************************
     *              EDIT ME
     ***************************************************/
...
function StormOnDemand_loadAsset($assetPath, $vars = array())
{
    $str = '';
...
function StormOnDemand_loadTemplates() {
...
function StormOnDemand_validateArray($array, $pregs) {
