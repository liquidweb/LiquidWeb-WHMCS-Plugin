	    ...
	    {if $sbs}
            {foreach from=$sbs item=s}
                <tr>
                    ...
                    <td class="status" {if $template eq "six" or $custom_template eq "YES"}style="margin-top:10px;background-color:#fafafa;text-align: center;"{/if}>{$s.status}</td>
                    <td class="actions"  style="text-align: center;">
                       {if !$s.is_assigned}
                       <a href="{$request_uri}&ajaxaction=attach&uid={$s.sbs.uniq_id}" {if $template eq "six" or $custom_template eq "YES"}style="height:30px !important;"{/if} class="btn btn-success send-ajax-req">Attach</a>
                       {else}
                       <a href="{$request_uri}&ajaxaction=detach&uid={$s.sbs.uniq_id}" {if $template eq "six" or $custom_template eq "YES"}style="height:30px !important;"{/if} class="btn btn-danger send-ajax-req">Detach</a>
                       {/if}
                    </td>
                </tr>
            {/foreach}
...
var pInterval = null;
 ...
 jQuery('a.send-ajax-req').each(function(){
 	jQuery(this).click(function(e){
 		...
 		jQuery(this).parent().append('<br/><span class="whait-loader"><img src="modules/servers/StormOnDemandPrivateParent/assets/images/loading.gif"alt="loading..." class="preolader"/></span>');
