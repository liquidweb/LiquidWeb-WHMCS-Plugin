    ...
    {literal}
        $(function(){
            ...
            $("#add_rule").click(function(event){
                ...
                $("#table_rules tbody").append('<tr><td><input type="text" name="advanced['+max+'][label]" value="" style="width: 100%"/></td><td><input type="text" name="advanced['+max+'][source_ip]" value="" style="width: 100%"/></td><td><input type="text" name="advanced['+max+'][destination_ip]" value="" style="width: 100%"/></td><td><input type="text" name="advanced['+max+'][destination_port]" value="" style="width: 100%"/></td><td><select name="advanced['+max+'][protocol]"><option value="any">Any</option><option value="tcp">TCP</option><option value="udp">UDP</option><option value="icmp">ICMP</option></select></td><td><select name="advanced['+max+'][action]"><option value="allow">Allow</option><option value="deny">Deny</option></select></td><td><a href="#" class="remove btn btn-danger btn-small">Remove</a></td></tr>');
    ...
    <div id="basic_firewall" {if $type != 'basic'} style="display: none; margni-top: 10px"{else} style="margni-top: 10px" {/if}>
        ...
        <ul style="overflow: hidden; margin-top: 5px">
            {foreach from=$options item=option}
                <li style="width: 33%; float: left; list-style-type: none"><input style="margin-right: 3px" type="checkbox" name="basic_opt[{$option}]" value="1" {if $rules[$option]}checked="checked"{/if} />{$option}</li>
            {/foreach}
        </ul>
    </div>

    <div id="advanced_firewall" {if $type != 'advanced'} style="display: none; margin-top: 10px"{else} style="margin-top: 10px"{/if}>
        <h3>Advanced Configuration</h3>
        <table class="table table-framed table table-condensed" style="width: 100%; margin-top: 5px" id="table_rules">
            <thead>
                <tr>
                    <th>Label</th>
                    <th>Source IP</th>
            ...
            <tbody>
                {foreach from=$advanced_rules item=option key=k}
                    <tr>
                        ...
                        <td><a href="#" class="remove btn btn-danger btn-small">Remove</a></td>
                    </tr>
                {/foreach}
            </tbody>
        </table>
        <a href="#" id="add_rule" class="btn btn-success">Add Rule</a>
    </div>
