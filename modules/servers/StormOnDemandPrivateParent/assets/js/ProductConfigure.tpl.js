jQuery(function(){
	...
	function checkQuota(){
			...
			jQuery('select[name="{$inpt_bandwidth_quota_name}"]').parent().append('<img src="../modules/servers/StormOnDemandPrivateParent/assets/images/admin/loading.gif" class="preloader" alt="loading..."/>');
			
			
			//change Backup Quota
			jQuery('select[name="{$inpt_backup_quota_name}"]').html('').removeAttr('disabled');
			jQuery('select[name="{$inpt_backup_quota_name}"]').parent().append('<img src="../modules/servers/StormOnDemandPrivateParent/assets/images/admin/loading.gif" class="preloader" alt="loading..."/>');
			
			//getConfig
			jQuery.post(urlToScript + '&stormajax=load-quota', function(data){
				if(data && data !== ''){
				    try{
