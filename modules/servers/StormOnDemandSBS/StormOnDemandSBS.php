...
include_once dirname(__FILE__) .DIRECTORY_SEPARATOR.'StormOnDemandSBS_Loader.php';
...
if(file_exists(dirname(__FILE__).DS.'moduleVersion.php')){
    ...
    define('STORM_ON_DEMAND_SBS_VERSION', $moduleVersion);
}else{
     define('STORM_ON_DEMAND_SBS_VERSION', 'Development Version');
}

function StormOnDemandSBS_MetaData()
{
    return array(
        'DisplayName' => '(Deprecated) StormOnDemandSBS ',
    );
}
function StormOnDemandSBS_checkConnection()
{
 ...
 * @return array
 */
function StormOnDemandSBS_ConfigOptions($loadValuesFromServer = true) {
    ...
    }elseif($_REQUEST['stormajax'] == 'generate-confoption')
    {
        ...
        $groups[] = array
        (
            'Name'          =>  'Configurable Options For Storm On Demand SBS Module',
                ...
                jQuery(function(){
                  jQuery( document ).ready(function() {
                    jQuery(".load-configuration").click(function(event){
                        ...
                        $("#conf-dialog").html("<p style=\"text-align:center\"><img src=\"../modules/servers/StormOnDemandSBS/assets/images/admin/loading.gif\" alt=\"loading...\"/></p>");
    ...
    $newVersion = StormOnDemandSBSForWHMCS_getLatestVersion();
    ...
    if($script == 'configproducts.php'){
        $testConnection = StormOnDemandSBS_checkConnection();
    }else{
        $testConnection = true;
    }
 ...
 * @return string
 */
function StormOnDemandSBS_CreateAccount($params) {

    try{
          $username           =   StormOnDemandSBS_getOption('Username', $params);
          $password           =   StormOnDemandSBS_getOption('Password', $params);
          $zone =   $params['configoptions']['zone'] ? $params['configoptions']['zone'] : StormOnDemandSBS_getOption('Zone', $params);
          $size =   $params['configoptions']['size'] ? $params['configoptions']['size'] : StormOnDemandSBS_getOption('Size', $params);
          $crossAttaching = (boolean) (isset($params['configoptions']['crossAttaching']) ? $params['configoptions']['crossAttaching'] : StormOnDemandSBS_getOption('Cross Attaching', $params));
          ...
          if(StormOnDemandSBS_getOption('Cross Attaching', $params) == null && !isset($params['configoptions']['crossAttaching'])){
 ...
 * @return string
 */
function StormOnDemandSBS_ChangePackage($params) {
      ...
      try{
            ...
            $storage = new StormOnDemand_Storage($username, $password, 'bleed');
            $storage->setDebug(true);
            $newSize = $params['configoptions']['size'] ? $params['configoptions']['size'] : StormOnDemandSBS_getOption('Size', $params);
 ...
 * @return string
 */
function StormOnDemandSBS_TerminateAccount($params) {
      if (!$params['customfields']['uniq_id'])
		return 'Custom Field /Uniq ID/ is empty';
      try{
            $username           =   StormOnDemandSBS_getOption('Username', $params);
...
function StormOnDemandSBS_getOption($option, $params)
{
    $config = StormOnDemandSBS_ConfigOptions();
...
function StormOnDemandSBS_ClientArea($params) {
      $module =  basename(dirname(__FILE__)); //Eg. module name is StormOnDemandSBS
      try {

             $clientArea = new StormOnDemandSBS($params['serviceid']);
...
//Register instance
StormOnDemandSBSForWHMCS_registerInstance();
function StormOnDemandSBSForWHMCS_registerInstance()
{
    /****************************************************
     *              EDIT ME
     ***************************************************/
    //Set up name for your module.
    $moduleName         =   "Storm On Demand SBS For WHMCS";
    //Set up module version. You should change module version every time after updating source code.
    $moduleVersion      =   STORM_ON_DEMAND_SBS_VERSION;
    //Encryption key
...
function StormOnDemandSBSForWHMCS_getLatestVersion()
{
    /****************************************************
     *              EDIT ME
     ***************************************************/
