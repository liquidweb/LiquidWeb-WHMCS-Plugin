...
if(!class_exists('LiquidWebPrivateParent_resources'))
{
    class LiquidWebPrivateParent_resources extends SBResource 
    {
        //module name
        const name = 'Liquid Web Private Parent';
        ...
        public function getProductAccounts()
        {
            ...
            $settings = array();
            $rows = mysql_get_array("SELECT setting, value FROM mg_LiquidWebPrivateParentProduct WHERE product_id = ? AND (setting = 'Password' OR setting='Username')", array($this->product_id));
            foreach($rows  as $set)
            {
